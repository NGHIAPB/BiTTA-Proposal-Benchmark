...
def main():
    ...
    ########## Dataset loading ############################
    # source model (huan luyen nguon, dung de tao checkpoint)
    if conf.args.method == 'Src':
        ...
        raise NotImplementedError(conf.args.method)

    corruption_list = []

    # Setting theo kich ban thuc nghiem:
    ...
    for corruption in corruption_list:
        ...
        # imagenetR: TTA khong can du lieu nguon (ResNet-18 pretrained torchvision). colored-mnist chi can nguon khi
        # huan luyen checkpoint (--method Src). Bo qua o cac truong hop con lai de khoi quet du lieu vo ich.
        if conf.args.dataset != "imagenetR" and not (conf.args.dataset == "colored-mnist" and conf.args.method != "Src"):
        ...
        if hasattr(learner, 'target_train_set'):
            ...
            data_loader.delete_target_cache(filename, corruption_name, file_path, transform=None)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--dataset', type=str, default='',
                        help='cifar10 | cifar100 | tiny-imagenet | pacs | imagenetR | colored-mnist')
    ...
    # BiTTA: khoi phuc ngau nhien ve trong so nguon sau moi lan cap nhat (0 = tat)
    parser.add_argument('--restoration_factor', type=float, default=0.0,
                        help='xac suat khoi phuc trong so ve mo hinh nguon (BiTTA)')
    ...
    parser.add_argument('--turn_off_reset', action='store_true', default=False,
                        help="turn off default resetting algorithm of SAR")
    ...
    parser.add_argument('--diversity_topn', type=int, default=10,
                         help='Số ứng viên uncertain nhất được xét trước khi chọn k mẫu đa dạng (Hybrid Uncertainty)')

    parser.add_argument('--n_dropouts', type=int, default=4)
    parser.add_argument('--dropout_rate', type=float, default=-1)

    # --- C1: information-maximizing selection (--sample_selection info_max) ---
    # Expected information gain of one binary-feedback query is h_b(p1), the binary
    # entropy of the top-1 posterior, which peaks at p1=0.5 rather than p1->0.
    parser.add_argument('--n_dropouts_select', type=int, default=-1,
                        help='MC-dropout passes used ONLY for active selection. -1 => reuse --n_dropouts.')
    parser.add_argument('--explore_eps', type=float, default=0.0,
                        help='C1: probability each query slot is spent on a random sample instead of the '
                             'score-maximizing one. Counters the censoring of the Platt calibration buffer.')
    parser.add_argument('--platt_min_samples', type=int, default=60,
                        help='C1: feedback pairs required before info_max engages; until then it falls back '
                             'to mc_conf ranking.')
    parser.add_argument('--platt_buffer_size', type=int, default=300,
                        help='C1: max (confidence, binary-outcome) pairs kept for calibration. Bounded because '
                             'the net is non-stationary.')
    parser.add_argument('--platt_refit_every', type=int, default=1,
                        help='C1: refit the Platt map every N selection rounds.')
