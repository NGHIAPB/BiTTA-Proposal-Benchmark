...
# BO SUNG: noi DeYO voi 5 dataset cua ho BiTTA-family (CIFAR-10-C, CIFAR-100-C, Tiny-ImageNet-C, PACS,
# ImageNet-R) de Continuous/Mixed shift dung chung 1 bo du lieu/checkpoint da chuan bi cho ca 5 phuong
# phap kia -- xem chu thich dau file dataset/scenario_dataset.py.
from dataset.scenario_dataset import ScenarioValDataset, SCENARIO_UNITS, build_scenario_model

SCENARIO_DSETS = set(SCENARIO_UNITS.keys())
    ...
    with torch.no_grad():
        ...
        for i, dl in enumerate(val_loader):
            ...
            if biased:
                ...
                acc1, acc5 = accuracy(output, target, topk=(1, min(5, args.num_class)))
...
if __name__ == '__main__':    
    ...
    if args.dset=='ImageNet-C':
        ...
        args.num_class = 2
    elif args.dset in SCENARIO_DSETS:
        args.num_class = {"CIFAR10-C": 10, "CIFAR100-C": 100, "TinyImageNet-C": 200,
                          "PACS-scenario": 7, "ImageNetR-scenario": 200, "WaterBirds-scenario": 2, "ColoredMNIST-scenario": 2}[args.dset]
    print('The number of classes:', args.num_class)

    if args.dset=='Waterbirds':
        assert biased
        assert args.data_corruption_file.endswith('h5py')
        # SUA: cho phep them resnet18_bn (ban goc chi cho resnet50_bn_torch).
        # CANH BAO: ResNet-18 + Waterbirds CHUA duoc paper DeYO kiem chung (paper
        # chi bao cao ResNet-50-BN cho Waterbirds, Table 3) -- khong coi la tai lap paper.
        assert args.model in ('resnet50_bn_torch', 'resnet18_bn')
    elif args.dset=='ColoredMNIST':
        assert biased
        assert args.model == 'resnet18_bn'
    elif args.dset in SCENARIO_DSETS:
        # BO SUNG: CIFAR-10-C/100-C, Tiny-ImageNet-C, PACS, ImageNet-R -- dung mo hinh + checkpoint
        # cua ho BiTTA-family (xem build_scenario_model), khong phai resnet18_bn/resnet50_bn_torch.
        assert args.model == 'resnet18_scenario'
        assert biased == (args.dset == "WaterBirds-scenario")
    if biased:
        assert args.dset in ("Waterbirds", "ColoredMNIST", "WaterBirds-scenario")
    ...
    if args.dset in SCENARIO_DSETS:
        common_corruptions = [args.unit] if args.unit else list(SCENARIO_UNITS[args.dset])
        assert all(u in SCENARIO_UNITS[args.dset] for u in common_corruptions), common_corruptions

    if args.exp_type == 'mix_shifts' and (args.dset=='ImageNet-C' or args.dset in SCENARIO_DSETS):
        datasets = []
        for cpt in common_corruptions:
            ...
            if args.dset in SCENARIO_DSETS:
                val_dataset = ScenarioValDataset(args.dset, cpt)
            else:
    ...
    if args.continual:
        # SUA: ban goc dung args.score_type nhung khong co dong add_argument nao
        # dinh nghia field nay trong config.py (ca ban goc cua tac gia) -- gay
        # AttributeError ngay khi args.continual=True, bat ke co bat wandb_log hay
        # khong (run_name duoc tinh vo dieu kien). Doi sang args.method, chi anh
        # huong ten hien thi tren wandb, khong lien quan thuat toan.
        run_name = f'{args.method}_{args.plpd_threshold}_{args.method}_continual_{date_time}'
    ...
    if args.method in ['tent', 'eata', 'sar', 'deyo', 'no_adapt']:
        if args.model == "resnet50_gn_timm":
            ...
            # SUA: ban goc chi ho tro ColoredMNIST (pickle co san); bo sung nhanh
            # tong quat cho cac dataset con lai. Chi la lua chon kien truc mang,
            # khong dung den thuat toan EATA/DeYO/TENT/SAR.
            if args.dset == 'ColoredMNIST':
                with open(os.path.join(args.data_corruption, args.cmmodel_name), 'rb') as f:
                    net_ewc = pickle.load(f)
            else:
                net_ewc = Resnet.__dict__['resnet18'](pretrained=True)
        elif args.model == "resnet18_scenario":
            net_ewc = build_scenario_model(args.dset, args.seed, 'cpu')
        else:
            assert False, NotImplementedError
    ...
    for corrupt_i, corrupt in enumerate(common_corruptions):
        ...
        if args.method in ['tent', 'eata', 'sar', 'deyo', 'no_adapt']:
            if (args.corruption != 'mix_shifts'):
                if args.dset=='ImageNet-C':
                    ...
                    val_dataset = ScenarioValDataset(args.dset, args.corruption)
        ...
        # SUA (Continuous): ban goc dung lai mo hinh/optimizer tu checkpoint cho MOI corruption bat ke
        # --continual, nen "continual" thuc chat chay nhu Fully TTA. Khi continual va khong phai corruption
        # dau, giu nguyen net/optimizer/adapt_model cua vong truoc (trang thai thich nghi duoc mang tiep).
        _reuse = bool(args.continual) and corrupt_i > 0
        # build model for adaptation
        if _reuse:
            pass
        elif args.method in ['tent', 'eata', 'sar', 'deyo', 'no_adapt']:
            if args.model == "resnet18_scenario":
                net = build_scenario_model(args.dset, args.seed, 'cpu')
                ...
                # SUA: xem giai thich o nhanh net_ewc phia tren.
                if args.dset == 'ColoredMNIST':
                    with open(os.path.join(args.data_corruption, args.cmmodel_name), 'rb') as f:
                        net = pickle.load(f)
                else:
                    net = Resnet.__dict__['resnet18'](pretrained=True)
                args.lr = (0.00025 / 64) * bs * 2 if bs < 32 else 0.00025
                args.lr *= args.lr_mul
            else:
                assert False, NotImplementedError
        ...
        if args.method == "tent":
            ...
            # Continuous: Fisher tinh 1 lan tren mo hinh nguon o corruption dau (tinh lai tren mo hinh da
            # thich nghi se sai y nghia EATA), sau do giu nguyen adapt_model.
            if _reuse:
                pass
            elif args.eata_fishers:
                print('EATA!')
                ...
                if args.dset=='Waterbirds':
                    ...
                    # giong ho BiTTA-family: dung domain dich dau tien de tinh Fisher
                    fisher_dataset = ScenarioValDataset(args.dset, SCENARIO_UNITS[args.dset][0])
                    fisher_loader = torch.utils.data.DataLoader(fisher_dataset, batch_size=args.test_batch_size,
                                                                shuffle=args.if_shuffle, num_workers=args.workers,
                                                                pin_memory=True)
                else:
