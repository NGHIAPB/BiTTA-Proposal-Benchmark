...
def delete_target_cache(dataset, cond, data_file_path, transform=None):
    """Xoa file cache target_train_set ngay sau khi da dung xong trong 1 lan chay.
    Voi --tgt cont, moi corruption chi duyet qua dung 1 lan nen cache nay khong
    bao gio duoc doc lai trong cung 1 tien trinh -> xoa ngay de tranh day dia."""
    root = './cached_data/'
    dir = root + str(dataset) + '/'
    if len(cond) > 10 and isinstance(cond, list):
        str_cond = "rand"
    ...
    try:
        if os.path.isfile(cache_path):
            ...
            print(f'Cache cleaned up (disk-saving): {cache_path}')
    except Exception:
        pass
